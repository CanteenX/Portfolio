...
const GlassCard = React.forwardRef<HTMLDivElement, GlassCardProps>(
  ({ className, title = "Project Name", description = "Create, share, and use beautiful custom elements made with CSS.", href, icon, ...props }, ref) => {
    return (
      <div
        ref={ref}
      ...
      >
        <div className="relative h-full rounded-[50px] bg-gradient-to-br from-zinc-900 to-black shadow-2xl transition-all duration-500 ease-in-out [transform-style:preserve-3d] group-hover:[box-shadow:rgba(0,0,0,0.3)_30px_50px_25px_-40px,rgba(0,0,0,0.1)_0px_25px_30px_0px] group-hover:[transform:rotate3d(1,1,0,30deg)]">
          ...
          <div className="absolute bottom-5 left-5 right-5 flex items-center justify-end [transform-style:preserve-3d] [transform:translate3d(0,0,26px)]">
            {href ? (
              <Link href={href} className="flex cursor-pointer items-center gap-1 transition-all duration-200 ease-in-out hover:[transform:translate3d(0,0,10px)]">
                <span className="text-xs font-bold text-white">View more</span>
                <ChevronDown className="h-4 w-4 stroke-white" strokeWidth={3} />
              </Link>
            ) : (
              <div className="flex cursor-pointer items-center gap-1 transition-all duration-200 ease-in-out hover:[transform:translate3d(0,0,10px)]">
                <span className="text-xs font-bold text-white">View more</span>
                <ChevronDown className="h-4 w-4 stroke-white" strokeWidth={3} />
              </div>
            )}
          ...
          <div className="absolute top-0 right-0 [transform-style:preserve-3d]">
            ...
            >
              {icon || <ULogo className="w-5 fill-black" />}
            </div>
          </div>
        </div>
      </div>
